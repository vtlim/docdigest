...
### Summaries or meta descriptions

Summaries are generated by `summarize.py`, and meta descriptions are generated with `meta_description.py`.
They follow the same pattern:

- Both have `estimate_*_costs(parsed_docs, llm)` functions
- Both have generic functions (`summarize(llm, ...)` and `generate_meta(llm, ...)`)
- Both support `llm="none"` for dry-run mode and `llm="claude"` for API calls
- Both can be easily extended to support additional LLMs like GPT
